...
rwmd_model <- RWMD$new(glove.model$get_word_vectors(), method = "cosine")
...
#embeddings.train.vector.rwmd <- sim.rwmd.embeddings.matrix(sentences_train[,1:2], word_vectors = glove.model$get_word_vectors(), rwmd_model = rwmd_model, checkDistance = TRUE, regularization = "none")
#embeddings.test.vector.rwmd <- sim.rwmd.embeddings.matrix(sentences_test[,1:2], word_vectors = glove.model$get_word_vectors(), rwmd_model = rwmd_model, checkDistance = TRUE, regularization = "none")
...
write_lines(toJSON(data.train), "Datasets/mainscript-run-clear-train.json")
write_lines(toJSON(data.test), "Datasets/mainscript-run-clear-test.json")

clearRunResults <- experiment(data.train, data.test, 1:ncol(data.train))
write.table(clearRunResults, "Results/clear-run-results-original.csv", sep = ";", quote = TRUE, fileEncoding = "UTF-8", append = FALSE)