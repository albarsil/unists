...
data.train <- fromJSON(read_lines("Datasets/Datasets/mainscript-run-clear-train.json"))
data.test <- fromJSON(read_lines("Datasets/Datasets/mainscript-run-clear-test.json"))
...
clearRunResults <- experiment.glm(data.train, data.test, 1:ncol(data.train))
write.table(clearRunResults, "Results/clear-run-results-glm.csv", sep = ";", quote = TRUE, fileEncoding = "UTF-8", append = FALSE)