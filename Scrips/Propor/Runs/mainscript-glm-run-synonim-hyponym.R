...
# write_lines(toJSON(data.test), "Datasets/mainscript-run-synonim-hyponym-test.json")

library(readr)
library(jsonlite)

data.train <- fromJSON(read_lines("Datasets/Datasets/mainscript-run-synonym-hyponym-train.json"))
data.test <- fromJSON(read_lines("Datasets/Datasets/mainscript-run-synonim-hyponym-test.json"))
...
write.table(clearRunResults, "Results/glm-run-results-synonym-hyponym.csv", sep = ";", quote = TRUE, fileEncoding = "UTF-8")