...
# No total levará 7 * 3 = 21 dias para execucao em um computador, pois 
# Ou uma semana em tres computadores.
# O server executará a taxa de aprendizado 0.12
# Os demais devem se dividir

initTime <- Sys.time()
for (i in lneurons) {
      # ... make some data
    datalist[[i]] <- experiment.rna(data.train, data.test, 1:ncol(data.train), hidden_neurons = i, learning_rate = 0.12)
    print(sprintf("N: %s - LR: %s", i, 0.12)) 
}


#clearRunResults = experiment.rna(data.train, data.test, 1:ncol(data.train), hidden_neurons = 5, learning_rate = 0.01)


clearRunResults = do.call(rbind, datalist)
print(difftime(Sys.time(), initTime))
write.table(clearRunResults, "Results/clear-run-results-rna-1-hidden.csv", sep = ";", quote = TRUE, fileEncoding = "UTF-8", append = FALSE)