...
data.train <- fromJSON(read_lines("Datasets/mainscript-run-synonym-train.json"))
data.test <- fromJSON(read_lines("Datasets/mainscript-run-synonym-test.json"))
...
write.table(clearRunResults, "Results/grnn-run-results-synonym.csv", sep = ";", quote = TRUE, fileEncoding = "UTF-8")